...
namespace RecrutamentoRobo.Infrastructure.Services
{
    ...
    {
        public GraphEmailService(ILogger<GraphEmailService> logger, IConfiguration configuration, GraphServiceClient graphClient)
        {
            _logger = logger;
            _configuration = configuration;

            // Pega as configurações do appsettings.json
            var tenantId = _configuration["EmailMonitor:TenantId"];
            var clientId = _configuration["EmailMonitor:ClientId"];


            // A autenticação interativa que usamos no teste.
            // O usuário precisará fazer login no navegador na primeira vez.
            var options = new DeviceCodeCredentialOptions
            {
                AuthorityHost = AzureAuthorityHosts.AzurePublicCloud,
                ClientId = clientId,
                TenantId = tenantId,
                // Callback para exibir a informação de login no console
            ...
            // Cria o cliente do Graph
            _graphClient = new GraphServiceClient(deviceCodeCredential);
        }
        ...
        public async Task MarkEmailAsReadAsync(string messageId)
        {
            ...
            try
            {
                ...
                var emailUpdate = new Microsoft.Graph.Models.Message
                {
                    IsRead = true
                };
        ...
        public async Task<IEnumerable<EmailMessage>> GetUnreadEmailsAsync()
        {
            ...
            try
            {
                ...
                return messages.Value.Select(message => new EmailMessage(
                    message.Id,
                    message.Subject,
                    message.Attachments.Select(attachment => new Core.Entities.AttachmentInfo(attachment.Id, attachment.Name))
                ));
            }
            catch (Exception ex)
            {
        ...
        public async Task DownloadAttachmentAsync(string messageId, string attachmentId, string downloadPath)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogWarning("Anexo {AttachmentId} não é um FileAttachment ou não contém conteúdo.", attachmentId);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<string> GetAttachmentTextAsync(string messageId, string attachmentId)
        {
            ...
            if (attachment is not Microsoft.Graph.Models.FileAttachment fileAttachment || fileAttachment.ContentBytes == null)
            {
            ...
            try
            {
                ...
                {
                    _logger.LogWarning("Tipo de anexo não suportado para extração de texto: {FileName}", fileAttachment.Name);
                }
            }
            catch (Exception ex)
            {
